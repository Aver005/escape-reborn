...
- **Bukkit-YAML комментарии НЕ теряет** (проверено на Paper 26.1.2, копия
  реального `config.yml`): `load → set → save` оставляет и строчные, и хвостовые
  комментарии. Схлопывается только выравнивание перед `#` (`1200        # x` →
  `1200 # x`) и нормализуются числа (`0.10` → `0.1`). Старое утверждение в
  `docs/guides/02-configs.md` было неверным и исправлено. А вот **рукописный
  формат предметов** в loot/traders действительно уезжает в сериализованный —
  это отдельная, настоящая проблема.
...
- **Смоук-тест сервера: `echo stop | ./start.bat` не годится** — `stop` доходит
  до консоли раньше загрузки миров и падает ванильным NPE
  (`CommandSourceStack.getLevel()` = null), сервер живёт до таймаута, а лог
  из-за буферизации stdout обрывается. Рабочая форма:
  `(sleep 75; echo stop; sleep 30) | timeout 280 ./start.bat > log 2>&1`.
